...
export default async function ProductDetailsPage({params}:{params:{id:string}}){
    ...
    const vendor=vendors[1] as Vendor
    ...
    const productImage = PRODUCT_IMG_DICT[product.image]
...
<div 
    className="w-16 h-16 rounded-lg bg-cover border-2 border-primary shrink-0" 
    style={{ backgroundImage: `url(${productImage})` }}
></div>
<div className="w-16 h-16 rounded-lg bg-cover bg-gray-200 shrink-0"></div>
<div className="w-16 h-16 rounded-lg bg-cover bg-gray-200 shrink-0"></div>
<div className="w-16 h-16 rounded-lg bg-cover bg-gray-200 shrink-0"></div>
</div>
</div>
...
<h1 className="text-[28px] font-bold leading-tight mb-2">{product.name}</h1>
<div className="flex items-center justify-between mb-4">
<div className="flex items-baseline gap-2">
<span className="text-2xl font-bold text-primary">{product.price}</span>
<span className="text-gray-400 line-through text-sm">N10235.06</span>
</div>
<div className="flex items-center gap-1">
<Star className=" text-yellow-400 filled-icon w-5"/>
